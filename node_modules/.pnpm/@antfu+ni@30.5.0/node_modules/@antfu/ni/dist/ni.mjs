...
import { A as require_prompts, T as formatPackageWithUrl, h as getCatalog, i as runCli, l as parseNi, o as getCommand, w as exclude } from "./src-DSImyIFd.mjs";
import fs from "node:fs";
import path from "node:path";
import process from "node:process";
import { styleText } from "node:util";
import { Fzf } from "fzf";
//#region node_modules/.pnpm/fast-npm-meta@2.2.0/node_modules/fast-npm-meta/dist/index.mjs
...
//#endregion
//#region src/catalog/bun.ts
function getWorkspacesField(json) {
	return json.workspaces != null && !Array.isArray(json.workspaces) ? json.workspaces : void 0;
}
function findBunWorkspaceRoot(cwd) {
	let dir = path.resolve(cwd);
	while (true) {
		const filePath = path.join(dir, "package.json");
		if (fs.existsSync(filePath)) try {
			if (JSON.parse(fs.readFileSync(filePath, "utf-8")).workspaces != null) return filePath;
		} catch {}
		const parent = path.dirname(dir);
		if (parent === dir) return null;
		dir = parent;
...
function readCatalogFields(json) {
	const workspaces = getWorkspacesField(json);
	if (workspaces?.catalog != null || workspaces?.catalogs != null) return {
...
const bunCatalogProvider = {
	async detect(cwd) {
		const filePath = findBunWorkspaceRoot(cwd);
		if (!filePath) return null;
		const content = fs.readFileSync(filePath, "utf-8");
		const { defaultCatalog, namedCatalogs } = readCatalogFields(JSON.parse(content));
		const catalogs = [];
		...
		if (hasNamedCatalogs) for (const [name, packages] of Object.entries(namedCatalogs)) catalogs.push({
			name,
			packages
		});
		return {
	...
	async addPackage(config, catalogName, pkgName, version) {
		...
		const workspaces = getWorkspacesField(json);
		const target = !!(workspaces && (workspaces.catalog != null || workspaces.catalogs != null)) ? workspaces : json;
		if (catalogName === "default") target.catalog = {
			...target.catalog ?? {},
			[pkgName]: version
		};
		else {
			target.catalogs = target.catalogs ?? {};
			target.catalogs[catalogName] = {
				...target.catalogs[catalogName] ?? {},
				[pkgName]: version
			};
		}
...
//#endregion
//#region node_modules/.pnpm/pnpm-workspace-yaml@1.7.0/node_modules/pnpm-workspace-yaml/dist/index.mjs
...
function getCatalogProvider(agent) {
	...
	if (agent === "bun") return bunCatalogProvider;
	return null;
}
//#endregion
...
function previousLabel(catalogName) {
	return catalogName === void 0 ? "skip" : catalogName;
}
async function promptSelectCatalog(config, pkgName, options = {}) {
	if (config.hasDefaultCatalog && !config.hasNamedCatalogs) return { catalogName: "default" };
	if (options.programmatic) return { catalogName: void 0 };
	...
	if (previous) {
		...
		if (hasRemaining) shortcutChoices.push({
	...
	if (catalog === SAME_AS_PREVIOUS) return { catalogName: previous.catalogName };
	if (catalog === APPLY_TO_REST) return {
...
function hasRemainingNewPackages(provider, config, packages, currentIndex) {
	for (let i = currentIndex + 1; i < packages.length; i++) if (!provider.findPackage(config, packages[i])) return true;
	return false;
}
async function handleCatalogInstall(agent, args, ctx) {
	...
	for (let i = 0; i < packages.length; i++) {
		...
		if (existing) {
			if (!ctx?.programmatic) console.log(`${styleText("green", "✓")} ${styleText("cyan", pkg)} ${styleText("dim", `→ found in ${existing.name} catalog`)}`);
			catalogEntries.push({
				name: pkg,
				catalogRef: getCatalogRef(existing.name)
			});
			continue;
		}
		let catalogName;
		if (applyToRest) catalogName = applyToRest.catalogName;
		else {
			...
			if (!ctx?.programmatic) {
				...
				if (selection.applyToRest) applyToRest = { catalogName };
			}
		}
		if (catalogName) {
			const version = await resolveVersion(pkg);
			await provider.addPackage(config, catalogName, pkg, version);
			if (!ctx?.programmatic) console.log(`${styleText("green", "+")} ${styleText("cyan", pkg)} ${styleText("dim", `→ ${catalogName} catalog (${version})`)}`);
			catalogEntries.push({
				name: pkg,
				catalogRef: getCatalogRef(catalogName)
			});
